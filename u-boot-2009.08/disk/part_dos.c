...
static inline int is_fat_partition(int part_type)
{
	return (part_type == 0x01 ||  /* FAT12 */
		part_type == 0x04 ||  /* FAT16 < 32M */
		part_type == 0x06 ||  /* FAT16 */
		part_type == 0x0b ||  /* W95 FAT32 */
		part_type == 0x0c ||  /* W95 FAT32 (LBA) */
		part_type == 0x0e);   /* W95 FAT16 (LBA) */
}
				 ...
				 int which_part, disk_partition_t *info)
{
	...
	for (i = 0; i < 4; i++, pt++) {
		...
		if ((pt->sys_ind != 0) &&
		    //(part_num == which_part) &&
		    is_fat_partition(pt->sys_ind) && /* FIXME:here find the first fat partition */
