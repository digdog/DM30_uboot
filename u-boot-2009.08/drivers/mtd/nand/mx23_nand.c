...
static void nand_BuildReadDma(NAND_dma_read_t * pChain, 
			      uint32_t u32NandDeviceNumber,
			      NAND_read_seed_t * pReadSeed)
{
	...
	pChain->wait_dma.bar = (apbh_dma_gpmi1_t *) & (pChain->timeout_dma);
	...
	if (pReadSeed->bEnableHWECC) {
		...
		if (pReadSeed->zNANDEccParams.u32ECCEngine == 0)	//ECC8
		{
			pChain->rx_data_dma.apbh_dma_gpmi6_u.
				...
				NAND_DMA_ECC_CTRL_PIO(0x0F,
					pReadSeed-> zNANDEccParams.u32EccType);
		} else {
			pChain->rx_data_dma.apbh_dma_gpmi6_u.
				apbh_dma_gpmi6_ctrl.gpmi_eccctrl.U =
		...
		pChain->rx_data_dma.bar = (void *) (((uint32_t) pReadSeed->pDataBuffer) & 0xFFFFFFFC);
		pChain->rx_data_dma.apbh_dma_gpmi6_u.apbh_dma_gpmi6_ctrl.gpmi_compare.U = 0x00;
	...
	pChain->rx_data_dma.apbh_dma_gpmi6_u.apbh_dma_gpmi6_ctrl.gpmi_ctrl0.U =
		NAND_DMA_RX_PIO(u32NandDeviceNumber, BV_GPMI_CTRL0_WORD_LENGTH__8_BIT, pReadSeed->uiReadSize);
	...
	pChain->rx_wait4done_dma.apbh_dma_gpmi3_u.apbh_dma_gpmi3_ctrl.gpmi_ctrl0.U =
		NAND_DMA_DISABLE_ECC_PIO(u32NandDeviceNumber);
	// Ignore the compare - we need to skip over it.
	pChain->rx_wait4done_dma.apbh_dma_gpmi3_u.apbh_dma_gpmi3_ctrl.gpmi_compare.U = 0x00;
	// Disable the ECC Block.
	pChain->rx_wait4done_dma.apbh_dma_gpmi3_u.apbh_dma_gpmi3_ctrl.gpmi_eccctrl.U =
			     ...
			     uint32_t u32ReadCode2)
{
	...
	pDmaReadSeed->tx_cle1_addr_dma_buffer.tx_cle1_addr_Columns_Rows.tx_cle1_Columns_Rows.tx_cle1_Type2.bType2Columns[0] =
			(uint8_t) (0);
	pDmaReadSeed->tx_cle1_addr_dma_buffer.tx_cle1_addr_Columns_Rows.tx_cle1_Columns_Rows.tx_cle1_Type2.bType2Columns[1] = 
			(uint8_t) (0);

	// Fill in the Row Address. (Can be 2 or 3 bytes)
	// Fill in the Column Address (Always 2 bytes)
	pDmaReadSeed->tx_cle1_addr_dma_buffer.tx_cle1_addr_Columns_Rows.tx_cle1_Columns_Rows.tx_cle1_Type2.bType2Rows[0] =
			(uint8_t) (0);
	pDmaReadSeed->tx_cle1_addr_dma_buffer.tx_cle1_addr_Columns_Rows.tx_cle1_Columns_Rows.tx_cle1_Type2.bType2Rows[1] =
			(uint8_t) (0);
	pDmaReadSeed->tx_cle1_addr_dma_buffer.tx_cle1_addr_Columns_Rows.tx_cle1_Columns_Rows.tx_cle1_Type2.bType2Rows[2] =
			(uint8_t) (0);
	...
	NAND_dma_read_t * pReadDmaDescriptor)
{
	...
	if (u32MaxCorrections) {
		...
		uint8_t *p8AuxPointer =
			(uint8_t *) pDmaReadSeed->pAuxBuffer;
		int i = 0;
		     ...
		     uint8_t *p8AuxillaryBuf)
{
	...
	//ecc type: RS_Ecc_4bit
	nand_InitReadDma(phy_DmaReadDescriptor, u32NumRowBytes, 8, 
			 pDmaReadSeed->zNANDEccParams.u32EccType, 0x00, 0x30);

	nfc_printf(NFC_DEBUG_MED, "Read NAND - NAND Read size %d\n", u32ReadSize);

	// Fill in the Column Address (Always 2 bytes)
	pDmaReadSeed->tx_cle1_addr_dma_buffer.tx_cle1_addr_Columns_Rows.tx_cle1_Columns_Rows.tx_cle1_Type2.bType2Columns[0] =
			(uint8_t) (u32ColumnOffset & 0xFF);
	pDmaReadSeed->tx_cle1_addr_dma_buffer.tx_cle1_addr_Columns_Rows.tx_cle1_Columns_Rows.tx_cle1_Type2.bType2Columns[1] =
			(uint8_t) ((u32ColumnOffset >> 8) & 0xFF);

	// Fill in the Row Address. (Can be 2 or 3 bytes)
	pDmaReadSeed->tx_cle1_addr_dma_buffer.tx_cle1_addr_Columns_Rows.tx_cle1_Columns_Rows.tx_cle1_Type2.bType2Rows[0] =
			(uint8_t) (u32PageNum & 0xFF);
	pDmaReadSeed->tx_cle1_addr_dma_buffer.tx_cle1_addr_Columns_Rows.tx_cle1_Columns_Rows.tx_cle1_Type2.bType2Rows[1] =
			(uint8_t) ((u32PageNum >> 8) & 0xFF);

	// This is always created, but the Address size determines whether
	// this data is actually sent.
	pDmaReadSeed->tx_cle1_addr_dma_buffer.tx_cle1_addr_Columns_Rows.tx_cle1_Columns_Rows.tx_cle1_Type2.bType2Rows[2] =
			(uint8_t) ((u32PageNum >> 16) & 0xFF);
