...
#define RSVBUFFADDR  	0x47000000	// unbufferable, uncacheable
#define BUFFER_SIZE   	0x10000

static NAND_ECC_Params_t nand_ecc_params;

/**
 * memequal - check if each byte of the @cs memory area equal to @value
 * @cs: One area of memory
 * @value: the value to be compare 
 * @count: The size of the area.
 */
static int memequal(const void * cs, uint8_t value, size_t count)
{
	const unsigned char *su1;
	int res = 0;

	for (su1 = cs; count > 0; ++su1, count--) {
		if ((res = *su1 - value) != 0) {
			printf("%s, count %d: 0x%x - 0x%x(value)\n", __func__, count, *su1, value);
			break;
		}
	}
	return res;
...
static void nand_ConfigurePinmux(void)
{
	uint32_t val;
...
static int nand_offset2pagenum(uint32_t offset, uint32_t *pagenum)
{
	if (offset % NF_PG_SZ) {
...
static int nand_FindGpmiCycles(uint32_t u32NandTime_ns,
...
//! \return void
////////////////////////////////////////////////////////////////////////////////
static void nand_GpmiSetNandTiming(void *pNewNANDTiming, uint32_t u32GpmiPeriod_ns)
{
	...
	// (Note that these are in units of GPMICLK cycles.)
	{
		...
		u32AddressSetup = nand_FindGpmiCycles(pNANDTiming->m_u8AddressSetup, u32GpmiPeriod_ns, 20);
		u32DataSetup = nand_FindGpmiCycles(pNANDTiming->m_u8DataSetup, u32GpmiPeriod_ns, 20);
		u32DataHold = nand_FindGpmiCycles(pNANDTiming->m_u8DataHold, u32GpmiPeriod_ns, 20);
		...
		u32DataSampleTime = nand_FindGpmiCycles((pNANDTiming->m_u8DSAMPLE_TIME +
					(u32GpmiPeriod_ns >> 2)), (u32GpmiPeriod_ns >> 1), 20) - 1;
...
static int nand_EnableGPMI(void)
{
	uint32_t val;
...
// for BCH
static void nand_SetFlashLayout(uint32_t u32NandDeviceNumber, NAND_ECC_Params_t * pnandecc)
{
	uint32_t val;
...
static void nand_ResetBCH(void)
{
	uint32_t val;
	// Bring out of reset and disable Clk gate.
...
static void nand_ResetDma(NAND_dma_reset_device_t * pChain,
	uint32_t u32NandDeviceNumber)
{
	...
	pChain->wait4rdy_dma.apbh_dma_gpmi1_u.apbh_dma_gpmi1_ctrl.gpmi_ctrl0.U =
	...
	pChain->tx_dma.apbh_dma_gpmi3_u.apbh_dma_gpmi3_ctrl.gpmi_ctrl0.U =
		NAND_DMA_COMMAND_PIO(u32NandDeviceNumber, NAND_RESET_DEVICE_SIZE, 0, ASSERT_CS);
	pChain->tx_dma.apbh_dma_gpmi3_u.apbh_dma_gpmi3_ctrl.gpmi_compare.U = (uint32_t) NULL;
	...
	pChain->sense_dma.apbh_dma_gpmi1_u.apbh_dma_gpmi1_ctrl.gpmi_ctrl0.U = 0;

	// Initialize the Terminator functions
	// Next function is null.
	pChain->success_dma.nxt = (apbh_dma_t *) 0x0;
	pChain->success_dma.cmd.U = ((uint32_t)(BF_APBH_CHn_CMD_IRQONCMPLT(1) |
	...
	pChain->timeout_dma.cmd.U = ((uint32_t)(BF_APBH_CHn_CMD_IRQONCMPLT(1) |
			BF_APBH_CHn_CMD_SEMAPHORE(1) |
			BV_FLD(APBH_CHn_CMD, COMMAND, NO_DMA_XFER)));
	pChain->timeout_dma.bar = (void *) 0x80508008;
}

static void nand_StartDma(void *pDmaChain, uint32_t u32NandDeviceNumber)
{
	uint32_t val;
	...
	HW_APBH_CHn_NXTCMDAR_WR(NAND0_APBH_CH + u32NandDeviceNumber, (uint32_t) pDmaChain);

	// Start DMA by incrementing the semaphore.
	BW_APBH_CHn_SEMA_INCREMENT_SEMA(NAND0_APBH_CH + u32NandDeviceNumber, 1);
}

static int nand_WaitDma(uint32_t u32uSecTimeout,
	uint32_t u32NandDeviceNumber)
{
	uint32_t val;
	int iComplete;
...
static void nand_BuildReadStatusDma(NAND_dma_read_status_t * pChain,
	uint32_t u32NandDeviceNumber,
	void *pBuffer)
{
	...
	pChain->success_dma.cmd.U = ((uint32_t)
...
static void nand_BuildReadDma(NAND_dma_read_t * pChain, uint32_t u32NandDeviceNumber,
	NAND_read_seed_t * pReadSeed)
{
	...
	pChain->tx_cle1_addr_dma.bar = pReadSeed->tx_cle1_addr_dma_buffer.tx_cle1_addr_buf;
	...
	pChain->tx_cle1_addr_dma.apbh_dma_gpmi3_u.apbh_dma_gpmi3_ctrl.gpmi_ctrl0.U =
		NAND_DMA_COMMAND_PIO(u32NandDeviceNumber, iCLE1_Size,
			BV_GPMI_CTRL0_ADDRESS_INCREMENT__ENABLED, ASSERT_CS);

	pChain->tx_cle1_addr_dma.apbh_dma_gpmi3_u.apbh_dma_gpmi3_ctrl.gpmi_compare.U = (uint32_t) NULL;

	pChain->tx_cle1_addr_dma.apbh_dma_gpmi3_u.apbh_dma_gpmi3_ctrl.gpmi_eccctrl.U =
		NAND_DMA_ECC_PIO(BV_GPMI_ECCCTRL_ENABLE_ECC__DISABLE);
	...
	pChain->tx_cle2_dma.bar = pReadSeed->tx_cle2_addr_dma_buffer.tx_cle2_buf;
	...
	pChain->tx_cle2_dma.apbh_dma_gpmi1_u.apbh_dma_gpmi1_ctrl.gpmi_ctrl0.U =
		NAND_DMA_COMMAND_PIO(u32NandDeviceNumber, 1, BV_GPMI_CTRL0_ADDRESS_INCREMENT__DISABLED,
			ASSERT_CS);
	...
	if (pReadSeed->bEnableHWECC) {
		...
		if (pReadSeed->zNANDEccParams.u32ECCEngine == 0)	//ECC8
		{
			pChain->rx_data_dma.apbh_dma_gpmi6_u.
				...
				NAND_DMA_ECC_CTRL_PIO(0x0F, pReadSeed->zNANDEccParams.u32EccType);
		} else {
			pChain->rx_data_dma.apbh_dma_gpmi6_u.apbh_dma_gpmi6_ctrl.gpmi_eccctrl.U =
				NAND_DMA_ECC_CTRL_PIO(pReadSeed->uiECCMask,
					BV_GPMI_ECCCTRL_ECC_CMD__DECODE_8_BIT);
		}
		pChain->rx_data_dma.apbh_dma_gpmi6_u.apbh_dma_gpmi6_ctrl.gpmi_ecccount.B.COUNT =
	...
	pChain->rx_data_dma.apbh_dma_gpmi6_u.apbh_dma_gpmi6_ctrl.gpmi_payload.
		U = (((uint32_t) pReadSeed->pDataBuffer) & 0xFFFFFFFC);
	// And the Auxiliary buffer here.
	pChain->rx_data_dma.apbh_dma_gpmi6_u.
		apbh_dma_gpmi6_ctrl.gpmi_auxiliary.U =
		(((uint32_t) pReadSeed->pAuxBuffer) & 0xFFFFFFFC);
	...
	pChain->timeout_dma.nxt = (apbh_dma_t *) 0x0;
	// Decrement semaphore, set IRQ, no DMA transfer.
	pChain->timeout_dma.cmd.U = ((uint32_t)
		(BF_APBH_CHn_CMD_IRQONCMPLT(1) |
		 BF_APBH_CHn_CMD_WAIT4ENDCMD(1) |
		 BF_APBH_CHn_CMD_SEMAPHORE(1) |
		 BV_FLD(APBH_CHn_CMD, COMMAND,
...
////////////////////////////////////////////////////////////////////////////////
static void nand_BuildQuickReadDma(NAND_dma_read_t * pChain,
	uint32_t u32NandDeviceNumber,
	NAND_read_seed_t * pReadSeed)
{
	...
	pChain->tx_cle1_addr_dma.apbh_dma_gpmi3_u.apbh_dma_gpmi3_ctrl.gpmi_ctrl0.B.CS = u32NandDeviceNumber;

	// Setup GPMI bus for second part of Read Command.  Need to set CLE
	// high, then send Read2 command (0x30), then clear CLE.
	// Only thing that needs to be set here is which NAND to talk to.
	//pChain->tx_cle2_dma.gpmi_ctrl0.B.CS = BF_GPMI_CTRL0_CS(u32NandDeviceNumber);
	pChain->tx_cle2_dma.apbh_dma_gpmi1_u.apbh_dma_gpmi1_ctrl.gpmi_ctrl0.B.CS = u32NandDeviceNumber;

	// Only thing that needs to be set here is which NAND to talk to.
	//pChain->wait_dma.gpmi_ctrl0.B.CS = BF_GPMI_CTRL0_CS(u32NandDeviceNumber);
	pChain->wait_dma.apbh_dma_gpmi1_u.apbh_dma_gpmi1_ctrl.gpmi_ctrl0.B.CS = u32NandDeviceNumber;

	if (pReadSeed->bEnableHWECC) {
		pChain->rx_data_dma.apbh_dma_gpmi6_u.apbh_dma_gpmi6_ctrl.gpmi_eccctrl.B.BUFFER_MASK = 
			pReadSeed->uiECCMask;
		pChain->rx_data_dma.apbh_dma_gpmi6_u.apbh_dma_gpmi6_ctrl.gpmi_ecccount.B.COUNT = 
			pReadSeed->uiReadSize;
	} else {
		...
		pChain->rx_data_dma.apbh_dma_gpmi6_u.apbh_dma_gpmi6_ctrl.gpmi_eccctrl.U = 0;
		pChain->rx_data_dma.apbh_dma_gpmi6_u.apbh_dma_gpmi6_ctrl.gpmi_ecccount.U = 0;
	}

	// Setup the data buffer.
	pChain->rx_data_dma.apbh_dma_gpmi6_u.apbh_dma_gpmi6_ctrl.gpmi_payload.U = 
		(((uint32_t) pReadSeed->pDataBuffer) & 0xFFFFFFFC);
	// And the Auxiliary buffer here.
	pChain->rx_data_dma.apbh_dma_gpmi6_u.apbh_dma_gpmi6_ctrl.gpmi_auxiliary.U =
		(((uint32_t) pReadSeed->pAuxBuffer) & 0xFFFFFFFC);
	...
	pChain->rx_data_dma.apbh_dma_gpmi6_u.apbh_dma_gpmi6_ctrl.gpmi_ctrl0.B.CS = u32NandDeviceNumber;
	pChain->rx_data_dma.apbh_dma_gpmi6_u.apbh_dma_gpmi6_ctrl.gpmi_ctrl0.B.XFER_COUNT = uiGPMITransferSize;

	// Disable the Chip Select and other outstanding GPMI things.
	pChain->rx_wait4done_dma.apbh_dma_gpmi3_u.apbh_dma_gpmi3_ctrl.gpmi_ctrl0.B.CS = u32NandDeviceNumber;
}


static void nand_InitReadDma(void *pReadDmaDescriptor,
	uint32_t u32NumRowBytes,
	uint32_t u32BusWidth,
	uint32_t u32ECCSize,
	uint32_t u32ReadCode1, uint32_t u32ReadCode2)
{
	...
	pDmaReadSeed->uiReadSize = (NF_PG_SZ + u32NumberOfECCbytes +
	...
	pDmaReadSeed->tx_cle1_addr_dma_buffer.tx_cle1_addr_Columns_Rows.tx_cle1 = (uint8_t) u32ReadCode1;
	pDmaReadSeed->tx_cle2_addr_dma_buffer.tx_cle2_addr_dma.tx_cle2 = (uint8_t) u32ReadCode2;

	// Fill in the Column Address (Always 2 bytes)
	pDmaReadSeed->tx_cle1_addr_dma_buffer.tx_cle1_addr_Columns_Rows.tx_cle1_Columns_Rows.tx_cle1_Type2.
		bType2Columns[0] = (uint8_t) (0);
	pDmaReadSeed->tx_cle1_addr_dma_buffer.
		tx_cle1_addr_Columns_Rows.tx_cle1_Columns_Rows.tx_cle1_Type2.
		bType2Columns[1] = (uint8_t) (0);

	// Fill in the Row Address. (Can be 2 or 3 bytes)
	// Fill in the Column Address (Always 2 bytes)
	pDmaReadSeed->tx_cle1_addr_dma_buffer.
		tx_cle1_addr_Columns_Rows.tx_cle1_Columns_Rows.tx_cle1_Type2.bType2Rows[0] = (uint8_t) (0);
	pDmaReadSeed->tx_cle1_addr_dma_buffer.
		tx_cle1_addr_Columns_Rows.tx_cle1_Columns_Rows.tx_cle1_Type2.bType2Rows[1] = (uint8_t) (0);
	pDmaReadSeed->tx_cle1_addr_dma_buffer.
		tx_cle1_addr_Columns_Rows.tx_cle1_Columns_Rows.tx_cle1_Type2.bType2Rows[2] = (uint8_t) (0);
...
static void nand_BuildProgramDma(NAND_dma_program_t * pChain,
	uint32_t u32NandDeviceNumber,
	uint32_t u32AddressSize,
	uint32_t u32DataSize,
	uint32_t u32EccSize, void *pWriteBuffer,
	void *pAuxBuffer)
{
	// CLE1 chain size is # columns + # Rows + CLE command.
	...
	pChain->tx_cle1_addr_dma.cmd.U = NAND_DMA_COMMAND_CMD(iCLE1_Size, 0, NAND_LOCK, 3) | (1 << 8);
	...
	pChain->tx_data_dma.apbh_dma_gpmi6_u.apbh_dma_gpmi6_ctrl.gpmi_ctrl0.U =
		NAND_DMA_TXDATA_PIO(u32NandDeviceNumber, BV_GPMI_CTRL0_WORD_LENGTH__8_BIT, 0);
	// Compare isn't used.
	pChain->tx_data_dma.apbh_dma_gpmi6_u.apbh_dma_gpmi6_ctrl.gpmi_compare.U = 0;

	pChain->tx_data_dma.apbh_dma_gpmi6_u.apbh_dma_gpmi6_ctrl.gpmi_eccctrl.U =
		NAND_DMA_ECC_CTRL_PIO(0x1ff, BV_GPMI_ECCCTRL_ECC_CMD__ENCODE_8_BIT);
	pChain->tx_data_dma.apbh_dma_gpmi6_u.apbh_dma_gpmi6_ctrl.gpmi_ecccount.U = u32DataSize;

	// Setup the data buffer.
	pChain->tx_data_dma.apbh_dma_gpmi6_u.apbh_dma_gpmi6_ctrl.gpmi_payload.U = 
		((uint32_t) (pWriteBuffer) & 0xFFFFFFFC);

	// And the Auxiliary buffer here.
	pChain->tx_data_dma.apbh_dma_gpmi6_u.apbh_dma_gpmi6_ctrl.gpmi_auxiliary.U =
		((uint32_t) (pAuxBuffer) & 0xFFFFFFFC);
	...
	pChain->tx_cle2_dma.nxt = (apbh_dma_gpmi1_t *) (&pChain->wait_dma);
	pChain->tx_cle2_dma.cmd.U = NAND_DMA_COMMAND_CMD(1, 0, NAND_LOCK, 3) | (1 << 8);;
	...
	// Set compare to NULL.
	pChain->tx_cle2_dma.apbh_dma_gpmi3_u.apbh_dma_gpmi3_ctrl.gpmi_compare.U = (uint32_t) NULL;

	// Disable the ECC.
	pChain->tx_cle2_dma.apbh_dma_gpmi3_u.apbh_dma_gpmi3_ctrl.gpmi_eccctrl.U = 0;
	//NAND_DMA_ECC_PIO(BV_GPMI_ECCCTRL_ENABLE_ECC__DISABLE);
	...
	pChain->statustx_dma.cmd.U = NAND_DMA_COMMAND_CMD(NAND_READ_STATUS_SIZE, 0, NAND_LOCK, 3) | (1 << 8);;
	pChain->statustx_dma.bar = &pChain->NandProgSeed.u8StatusCmd;
	pChain->statustx_dma.apbh_dma_gpmi3_u.apbh_dma_gpmi3_ctrl.gpmi_ctrl0.U =
		NAND_DMA_COMMAND_PIO(u32NandDeviceNumber, NAND_READ_STATUS_SIZE,
			BV_GPMI_CTRL0_ADDRESS_INCREMENT__DISABLED, ASSERT_CS);

	// Set compare to NULL.
	pChain->statustx_dma.apbh_dma_gpmi3_u.apbh_dma_gpmi3_ctrl.gpmi_compare.U = (uint32_t) NULL;

	// Disable the ECC.
	pChain->statustx_dma.apbh_dma_gpmi3_u.apbh_dma_gpmi3_ctrl.gpmi_eccctrl.U = 0;
	//NAND_DMA_ECC_PIO(BV_GPMI_ECCCTRL_ENABLE_ECC__DISABLE);
	...
	pChain->statusrx_dma.cmd.U = NAND_DMA_RX_NO_ECC_CMD(NAND_READ_STATUS_RESULT_SIZE, 0) | (1 << 8);;
	...
	pChain->statbranch_dma.bar = (apbh_dma_t *) (&pChain->program_failed_dma);
	...
	// Decrement semaphore, set IRQ, no DMA transfer.
	pChain->program_failed_dma.cmd.U = ((uint32_t)
		(BF_APBH_CHn_CMD_IRQONCMPLT(1)
		 | BF_APBH_CHn_CMD_WAIT4ENDCMD(1)
		 | BF_APBH_CHn_CMD_SEMAPHORE(1)
		 | BV_FLD(APBH_CHn_CMD, COMMAND, NO_DMA_XFER)));
	...
	printf("pChain->tx_cle1_addr_dma addr: 0x%x\n",
		(uint32_t) &pChain->tx_cle1_addr_dma);
	printf("pChain->tx_cle1_addr_dma.nxt: 0x%x\n",
		(uint32_t) pChain->tx_cle1_addr_dma.nxt);
	printf("pChain->tx_cle1_addr_dma.cmd: 0x%x\n",
		(uint32_t) pChain->tx_cle1_addr_dma.cmd.U);
	printf("pChain->tx_cle1_addr_dma.bar: 0x%x\n",
		(uint32_t) pChain->tx_cle1_addr_dma.bar);
	printf("bar data: 0x%x\n",
		*(uint32_t *) pChain->tx_cle1_addr_dma.bar);
	printf("pChain->tx_cle1_addr_dma ctrl0: 0x%x\n",
		(uint32_t) pChain->tx_cle1_addr_dma.apbh_dma_gpmi3_u.
		apbh_dma_gpmi3_ctrl.gpmi_ctrl0.U);
	...
	printf("pChain->tx_data_dma addr: 0x%x\n",
		(uint32_t) &pChain->tx_data_dma);
	printf("pChain->tx_data_dma.nxt: 0x%x\n",
		(uint32_t) pChain->tx_data_dma.nxt);
	printf("pChain->tx_data_dma.cmd: 0x%x\n",
		(uint32_t) pChain->tx_data_dma.cmd.U);
	printf("pChain->tx_data_dma.bar: 0x%x\n",
		(uint32_t) pChain->tx_data_dma.bar);
...
////////////////////////////////////////////////////////////////////////////////
static void nand_BuildEraseDma(NAND_dma_block_erase_t * pChain,
	uint32_t u32NandDeviceNumber,
...
static int rom_nand_hal_FindEccErrors(uint32_t u32ECCEngineType,
	uint32_t u32NumberOfCorrections)
{
	...
	if (u32ECCEngineType == 0) {
		...
		nfc_printf(NFC_DEBUG_MED, "Wait until ECC Complete Interrupt triggers.\n");
		...
		while (1) {
			...
			if (HW_BCH_CTRL_RD() & BM_BCH_CTRL_BM_ERROR_IRQ) {
				nfc_printf(NFC_DEBUG_MED, "BCH: AHB Bus interface Error\n");
				break;
			}
		}
...
static int nand_CheckECCStatus(uint32_t u32NandDeviceNumber, 
	uint32_t u32Threshold, NAND_dma_read_t * pReadDmaDescriptor)
{
	...
	uint32_t u32MaxCorrections = rom_nand_hal_FindEccErrors(pDmaReadSeed->zNANDEccParams.u32ECCEngine,
			u32Threshold);
	...
	if (u32MaxCorrections) {
		...
		uint32_t u32Temp;
		uint32_t uNumBlks = pDmaReadSeed->zNANDEccParams.m_u32NumEccBlocksPerPage + 1;

		// get the status of Blocks. 
		// Each block's status is in a byte, 
		// starts at the beginning of a new word where metadata ends
		indexToAuxBuffer = pDmaReadSeed->zNANDEccParams.m_u32MetadataBytes +
		...
		for (i = 0; i < uNumBlks; i++) {
			...
			if (i == 0)
				u32EccLevel = pDmaReadSeed->zNANDEccParams.m_u32EccBlock0EccLevel;
			else
			...
			if (u32Temp == BV_BCH_STATUS0_STATUS_BLK0__UNCORRECTABLE) {
				...
				nfc_printf(NFC_DEBUG_MAX, "BCH Status Sub-Block %d = 0x%X\n", i, u32Temp);
			}
		}
	}
...
////////////////////////////////////////////////////////////////////////////////
static int nand_Read(uint32_t u32NandDeviceNumber,
		     ...
		     uint8_t *p8AuxillaryBuf)
{
	...
	pDmaReadSeed->zNANDEccParams.m_u32NumEccBlocksPerPage = nand_ecc_params.m_u32NumEccBlocksPerPage;
	pDmaReadSeed->zNANDEccParams.m_u32MetadataBytes = nand_ecc_params.m_u32MetadataBytes;
	pDmaReadSeed->zNANDEccParams.m_u32EccBlock0Size = nand_ecc_params.m_u32EccBlock0Size;
	pDmaReadSeed->zNANDEccParams.m_u32EccBlockNSize = nand_ecc_params.m_u32EccBlockNSize;
	pDmaReadSeed->zNANDEccParams.m_u32EccBlock0EccLevel = nand_ecc_params.m_u32EccBlock0EccLevel;
	pDmaReadSeed->zNANDEccParams.m_u32EraseThreshold = nand_ecc_params.m_u32EraseThreshold;
	pDmaReadSeed->zNANDEccParams.u32EccType = nand_ecc_params.u32EccType;
	pDmaReadSeed->zNANDEccParams.m_u32PageSize = nand_ecc_params.m_u32PageSize;
	...
	nand_InitReadDma(phy_DmaReadDescriptor, u32NumRowBytes, 8,
		pDmaReadSeed->zNANDEccParams.u32EccType, 0x00, 0x30);	//ecc type: RS_Ecc_4bit

	nfc_printf(NFC_DEBUG_MED, "Read NAND - NAND Read size %d\n", u32ReadSize);

	// Fill in the Column Address (Always 2 bytes)
	pDmaReadSeed->tx_cle1_addr_dma_buffer.tx_cle1_addr_Columns_Rows.tx_cle1_Columns_Rows.tx_cle1_Type2.
		bType2Columns[0] = (uint8_t) (u32ColumnOffset & 0xFF);
	pDmaReadSeed->tx_cle1_addr_dma_buffer.  tx_cle1_addr_Columns_Rows.tx_cle1_Columns_Rows.tx_cle1_Type2.
		bType2Columns[1] = (uint8_t) ((u32ColumnOffset >> 8) & 0xFF);

	// Fill in the Row Address. (Can be 2 or 3 bytes)
	pDmaReadSeed->tx_cle1_addr_dma_buffer.tx_cle1_addr_Columns_Rows.tx_cle1_Columns_Rows.tx_cle1_Type2.
		bType2Rows[0] = (uint8_t) (u32PageNum & 0xFF);
	pDmaReadSeed->tx_cle1_addr_dma_buffer.  tx_cle1_addr_Columns_Rows.tx_cle1_Columns_Rows.tx_cle1_Type2.
		bType2Rows[1] = (uint8_t) ((u32PageNum >> 8) & 0xFF);

	// This is always created, but the Address size determines whether
	// this data is actually sent.
	pDmaReadSeed->tx_cle1_addr_dma_buffer.tx_cle1_addr_Columns_Rows.tx_cle1_Columns_Rows.tx_cle1_Type2.
		bType2Rows[2] = (uint8_t) ((u32PageNum >> 16) & 0xFF);
	...
	nand_BuildQuickReadDma(pDmaReadDescriptor, u32NandDeviceNumber, pDmaReadSeed);

	// Clear the ECC Complete flag.
	writel(0x1, BCH_CTRL_CLR);

	// Kick off the DMA.
	nand_StartDma((dma_cmd_t *) phy_DmaReadDescriptor, u32NandDeviceNumber);
	...
	if (status != SUCCESS) {
		...
		nfc_printf(NFC_DEBUG_MED, "NAND Read Complete, Check ECC Status.\n");
		nand_CheckECCStatus(u32NandDeviceNumber, pDmaReadSeed->zNANDEccParams.u32EccType,
...
int nandflash_read_buf(void *addr, void *data, int len)
{
	int status;
	int i;
	uint8_t *p8PageBuf, *phy_8PageBuf;
	uint8_t *p8AuxillaryBuf, *phy_8AuxillaryBuf;
	int size;
	uint32_t pageNum;

	nfc_printf(NFC_DEBUG_MED, "%s()\n", __FUNCTION__);

	p8PageBuf = (uint8_t *) (RSVBUFFADDR + BUFFER_SIZE);
	memset(p8PageBuf, 0x0, 0x2000);
	p8AuxillaryBuf = (uint8_t *) (RSVBUFFADDR + BUFFER_SIZE + 0x2000);
	...
	nand_offset2pagenum((uint32_t) addr, &pageNum);

	i = 0;

	while (size > 0) {
		nfc_printf(NFC_DEBUG_MAX, "Page Num: 0x%x", pageNum);
		status = nand_Read(0, 0, pageNum, NF_PG_SZ + flash_dev_info->spare_size,
		...
		if (status == SUCCESS) {
			...
			printf("nandflash read error\n");
		}

		if (size >= NF_PG_SZ) {
			memcpy((void *)((uint32_t) data + i * NF_PG_SZ), p8PageBuf, NF_PG_SZ);
		} else {
			memcpy((void *)((uint32_t) data + i * NF_PG_SZ), p8PageBuf, size);
		}
...
static int nand_Erase(uint32_t u32NandDeviceNumber, uint32_t BlockNum)
{
	int status;
	uint32_t wRowAddr;
	//    uint32_t wChipNum = pNANDDescriptor->wChipNumber;
	uint32_t u32BlockAddressBytes;
	uint32_t u32NumRowBytes = 3;

	NAND_dma_block_erase_t *pNandDmaDescriptor = (NAND_dma_block_erase_t *) RSVBUFFADDR;
	...
	pNandDmaDescriptor->NandEraseSeed.tx_block[0] = (uint8_t) (wRowAddr & 0xFF);
	pNandDmaDescriptor->NandEraseSeed.tx_block[1] = (uint8_t) ((wRowAddr >> 8) & 0xFF);
	pNandDmaDescriptor->NandEraseSeed.tx_block[2] = (uint8_t) ((wRowAddr >> 16) & 0xFF);
	...
	nand_StartDma((dma_cmd_t *) phy_DmaEraseDescriptor, u32NandDeviceNumber);
...
int nandflash_erase_block(void *block, uint32_t size)
{
	uint32_t BlockNum;

	printf("block: 0x%x size: 0x%x\n", (uint32_t) block, size);

	BlockNum = (uint32_t) block;
...
static int nand_Write(uint32_t u32NandDeviceNumber,
		      uint32_t u32PageNum,
		      uint8_t *p8PageBuf,
		      uint8_t *p8AuxillaryBuf)
{
	...
	uint32_t i, val;
	int rtCode;
	uint32_t btNumRowBytes = 0x3;

	// Create a pointer to this device.
	NAND_dma_program_t *pDmaWriteDescriptor = (NAND_dma_program_t *) RSVBUFFADDR;
	...
	pDmaWriteDescriptor->NandProgSeed.NandSizeVars.uiAddressSize = btNumRowBytes + 2;

	//u32TotalPageSize;
	pDmaWriteDescriptor->NandProgSeed.NandSizeVars.uiWriteSize = NF_PG_SZ + flash_dev_info->spare_size;

	// set the Word size
	// normally use 8 bit data width
	pDmaWriteDescriptor->NandProgSeed.NandSizeVars.uiWordSize = BV_GPMI_CTRL0_WORD_LENGTH__8_BIT;

	// Always 2 bytes and we're always going to be column 0.
	pDmaWriteDescriptor->NandProgSeed.bType2Columns[0] = (uint8_t) 0;
	pDmaWriteDescriptor->NandProgSeed.bType2Columns[1] = (uint8_t) 0;

	// Fill in the Row Address. (Can be 2 or 3 bytes)
	for (i = 0; i < btNumRowBytes; i++) {
		pDmaWriteDescriptor->NandProgSeed.bType2Rows[i] = (uint8_t) ((u32PageNum >> (8 * i)) & 0xFF);
	}
	...
	nfc_printf(NFC_DEBUG_MAX, "pDmaWriteDescriptor->NandProgSeed addr: 0x%x\n",
		(uint32_t) &pDmaWriteDescriptor->NandProgSeed);
	...
	/* Set for BCH */
	pDmaWriteDescriptor->NandProgSeed.zNANDEccParams.u32ECCEngine = nand_ecc_params.u32ECCEngine;
	pDmaWriteDescriptor->NandProgSeed.zNANDEccParams.m_u32NumEccBlocksPerPage =
	...
	pDmaWriteDescriptor->NandProgSeed.zNANDEccParams.m_u32EccBlock0EccLevel =
	...
	pDmaWriteDescriptor->NandProgSeed.zNANDEccParams.u32EccType = nand_ecc_params.u32EccType;
	pDmaWriteDescriptor->NandProgSeed.zNANDEccParams.m_u32PageSize = nand_ecc_params.m_u32PageSize;
	pDmaWriteDescriptor->NandProgSeed.bEnableHWECC = 1;	//enable ecc

	// Build the DMA that will program this sector.
	nand_BuildProgramDma(phy_DmaWriteDescriptor,
			     u32NandDeviceNumber,
	...
	nand_StartDma((dma_cmd_t *) phy_DmaWriteDescriptor, u32NandDeviceNumber);
...
int nandflash_program_buf(void *addr, void *data, int len)
{
	...
	while (size > 0) {
		...
		memcpy(p8PageBuf, (void *)((uint32_t) data + i * NF_PG_SZ), NF_PG_SZ);
...
int nand_ReadStatus(uint32_t u32NandDeviceNumber,
	uint32_t *pStatusResult)
{
	...
	*pStatusResult = (uint32_t) pdma_read_status->rx_Result;

	return retCode;
}

void nand_ReadID(uint32_t u32NandDeviceNumber, void *pReadIDBuffer)
{
	int status;
...
/*
 * nand_Init 只能在初始化的时候调用一次，如果多次初始化，则读写会出问题
 */
static int nand_Init(ReadIDCode * pReadIDBuf)
{
	int status;
	uint32_t val;
	uint32_t u32NandDeviceNumber = 0;
...
static void nand_test(void)
{
	...
	int i;
	uint32_t pageNum;
	uint8_t *pagebuf, *auxillarybuf;
	uint32_t StatusResult;

	printf("===========Write/Read TEST==============\n");
	nand_Erase(0, 0);

	pagebuf = (uint8_t *) (RSVBUFFADDR + BUFFER_SIZE);
	memset(pagebuf, 0x88, 0x2000);
	auxillarybuf = (uint8_t *) (RSVBUFFADDR + BUFFER_SIZE + 0x2000);
	memset(auxillarybuf, 0x00, 0x2000);

	pageNum = 0x000;
	for (i = 0; i < 20; i++) {
		memset(pagebuf, i, 0x2000);

		status = nand_Write(0, pageNum, pagebuf, auxillarybuf);
		if (status != SUCCESS)
			printf("Write Test Failed, 0x%x, page number 0x%x\n", i, pageNum);

		status = nand_Read(0, 0, pageNum, 2112, pagebuf, auxillarybuf);
		if (status != SUCCESS)
			printf("0x%x 0x%x 0x%x \n", *pagebuf, *(pagebuf + 1), *(pagebuf + 2));

		if (memequal(pagebuf, i, 2048) != 0)
			printf("nand data read error: 0x%x page number 0x%x\n", i, pageNum);
	...
	printf("===========Erase TEST==============\n");
	nand_Erase(0, 100);
	status = nand_Read(0, 0, 100, 2112, pagebuf, auxillarybuf);
	if (status != SUCCESS)
		printf("0x%x 0x%x 0x%x \n", *pagebuf, *(pagebuf + 1), *(pagebuf + 2));
	if (memequal(pagebuf, 0xff, 2048) != 0)
		printf("nand data read error: 0x%x page number 0x%x\n", i, pageNum);

	nand_ReadStatus(0, &StatusResult);
	printf("Return value2: 0x%x\n", StatusResult);
#if 0
	printf("===========Read Time TEST==============\n");
	uint32_t t1, t2;

	t1 = get_timer_masked();
	nandflash_read_buf(0, 0x40000000, 0x100000);
	t2 =  get_timer_masked();
	printf("%dM: %d ms\n", 1, t2 - t1);

	t1 = get_timer_masked();
	nandflash_read_buf(0, 0x40000000, 0x500000);
	t2 =  get_timer_masked();
	printf("%dM: %d ms\n", 5, t2 - t1);

	t1 = get_timer_masked();
	nandflash_read_buf(0, 0x40000000, 0xa00000);
	t2 =  get_timer_masked();
	printf("%dM: %d ms\n", 10, t2 - t1);

	t1 = get_timer_masked();
	nandflash_read_buf(0, 0x40000000, 0x1400000);
	t2 =  get_timer_masked();
	printf("%dM: %d ms\n", 20, t2 - t1);
...
static int nandflash_hwr_init(void)
{
	int i;
