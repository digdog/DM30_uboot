...
#include <malloc.h>
#include <asm/io.h>
#include <asm/arch/registers/regspinctrl.h>
#include <asm/arch/registers/regsbch.h>
#include <asm/arch/registers/regsgpmi.h>
#include <asm/arch/registers/regsclkctrl.h>
...
//	struct mtd_partition   *partitions;
	...
	int                    raw_oob_mode;
	int                    (*hooked_read_oob)(struct mtd_info *mtd, loff_t from, struct mtd_oob_ops *ops);
	int                    (*hooked_write_oob)(struct mtd_info *mtd, loff_t to, struct mtd_oob_ops *ops);
...
static inline void inline clk_gpmi_enable(void)
{
	uint32_t val;
	/* enable GPMI clock */
...
static inline void inline clk_gpmi_disable(void)
{
	uint32_t val;
	val = readl(HW_CLKCTRL_GPMI_ADDR);
	val |= (1<<31);
